...
export default async function KategorieDetailPage({ params, searchParams }: Props) {
  const { slug } = await params
  const {
    page = '1',
    marque = '',
  ...
  const breadcrumbItems = [
    ...
    { name: 'Kategorien', url: 'https://priesradio.ch/kategorien' },
    ...(isSubcat
      ? [
          { name: slugToLabel(slug[0]), url: `https://priesradio.ch/kategorien/${slug[0]}` },
          { name: categorie.nom, url: `https://priesradio.ch/kategorien/${fullSlug}` },
        ]
      : [{ name: categorie.nom, url: `https://priesradio.ch/kategorien/${fullSlug}` }]
    ),
  ]
      ...
      <section className="bg-[#0F172A] py-8 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative">
          <nav className="flex items-center gap-1.5 text-xs text-slate-500 mb-4 flex-wrap">
            ...
            {isSubcat && categorie.parent_slug && (
              <>
                <ChevronRight size={12} />
                <Link
                  href={`/kategorien/${categorie.parent_slug}`}
          ...
          <h1 className="text-2xl md:text-3xl font-bold" style={{ color: '#F1F5F9' }}>
            {categorie.nom} – Beste Preise {new Date().toLocaleDateString('de-CH', { month: 'long', year: 'numeric' })}
          </h1>
          <p className="mt-2 text-slate-400 text-sm leading-relaxed max-w-2xl">
            Vergleichen Sie die Preise in der Kategorie <span className="text-white font-medium">{categorie.nom}</span> bei den führenden Schweizer Online-Shops. {(meta?.total_items ?? 0) > 0 ? `${meta?.total_items} Produkt${(meta?.total_items ?? 0) !== 1 ? 'e' : ''}` : 'Produkte'} zu entdecken.
          </p>
        </div>
      </section>
