...
import { getProdukt, getProdukte } from '@/lib/api/produits'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const produit = await getProdukt(slug)
...
export default async function ProduktDetailPage({ params }: Props) {
  const { slug } = await params
  let produit: Produkt | null = null
  try { produit = await getProdukt(slug) } catch { notFound() }
  if (!produit) notFound()

  // Ähnliche Produkte (gleiche Kategorie, ohne aktuelles Produkt)
  let aehnliche: Produkt[] = []
  if (produit.categorie) {
    try {
      const res = await getProdukte({ categorie: produit.categorie })
  ...
  const jsonLd = {
    ...
    sku: produit.reference || produit.sku || undefined,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      ...(produit.categorie ? [
        { '@type': 'ListItem', position: 2, name: produit.categorie, item: `https://priesradio.ch/kategorien/${produit.categorie}` },
        { '@type': 'ListItem', position: 3, name: produit.nom, item: `https://priesradio.ch/produkt/${slug}` },
      ] : [
        { '@type': 'ListItem', position: 2, name: produit.nom, item: `https://priesradio.ch/produkt/${slug}` },
      ]),
    ],
  }
