...
def clean_search_query(query: str) -> str:
    ...
    query = re.sub(r'[^\w\s\-äöüÄÖÜßéèêëàâôùûçñ]', ' ', query)
...
def build_reference_pipeline(query: str, skip: int = 0, limit: int = 10) -> List[Dict]:
    ...
    return [
        {
            '$match': {
                'sku': {'$regex': f'^{re.escape(query)}$', '$options': 'i'}
            }
        },
        {
            '$addFields': {
...
def build_text_search_pipeline(query: str, num_words: int, skip: int = 0, limit: int = 10) -> List[Dict]:
    ...
    Nécessite un index Atlas Search nommé "Text" sur le champ `name`.
    """
...
def filter_by_relevance(raw_docs: List[Dict], query_words: List[str], num_words: int) -> List[Dict]:
    ...
    Vérifie que le nom contient suffisamment de mots de la requête :
    - 2 mots : ≥ 1 mot présent (50%)
    - 3-5 mots : ≥ 60%
    - 6+ mots : ≥ 30%

    Les docs sont triés par score de pertinence décroissant.
    Travaille sur les docs bruts MongoDB (champ `name`).
    """
    ...
    filtered = []
    for doc in raw_docs:
        title_lower = doc.get('name', '').lower()
